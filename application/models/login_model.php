...
class LoginModel
{
    ...
    public function login()
    {
        ...
        // There are other types of accounts that don't have passwords etc. 
        ...
        if (password_verify($_POST['user_password'], $result->user_password_hash)) {
			...
			Session::set('user_full_name', $result->user_full_name);
            Session::set('user_email', $result->user_email);
            Session::set('user_account_type', $result->user_account_type);
			Session::set('user_location', $result->user_location);
			Session::set('user_shift', $result->user_shift);
            Session::set('user_provider_type', 'DEFAULT');
			Session::set('user_password_reset_hash', $user_password_reset_hash);
			
			$qry = $this->db->prepare("SELECT `note_public`, `plant` FROM `notes` WHERE note_public = '1' AND plant = :plant");
			$qry->execute(array(":plant" => $result->user_location));
            ...
            $sql = "UPDATE users SET user_last_login_timestamp = :user_last_login_timestamp, user_password_reset_hash = :user_password_reset_hash WHERE user_id = :user_id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':user_id' => $result->user_id, ':user_last_login_timestamp' => $user_last_login_timestamp, ':user_password_reset_hash' => $user_password_reset_hash));
    ...
    public function loginWithCookie()
    {
        ...
        $query = $this->db->prepare("SELECT user_id, user_name, user_full_name, user_email, user_password_hash, user_active,
                                          user_account_type,  user_has_avatar, user_failed_logins, user_last_failed_login, user_location
                                     FROM users
    ...
    public function editUserName()
    {
	  ...
	  /**
     * Edit full name, provided in the editing form
     * @return bool success status
     */
    public function editFullName()
    {
        // new full name provided ?
        if (!isset($_POST['full_name']) OR empty($_POST['full_name'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_FULLNAME_FIELD_EMPTY;
            return false;
        }

        // new full name same as old one ?
        if ($_POST['full_name'] == $_SESSION["user_full_name"]) {
            $_SESSION["feedback_negative"][] = FEEDBACK_FULLNAME_SAME_AS_OLD_ONE;
            return false;
        }

        // full name cannot be empty and must be azAZ and 2-64 characters
        if (!preg_match("/^(?=.{2,64}$)[a-zA-Z ][a-zA-Z ]*(?: [a-zA-Z ]+)*$/", $_POST['full_name'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_FULLNAME_DOES_NOT_FIT_PATTERN;
            return false;
        }

        // clean the input
        $user_full_name = substr(strip_tags($_POST['full_name']), 0, 64);

        $query = $this->db->prepare("UPDATE users SET user_full_name = :user_full_name WHERE user_id = :user_id");
        $query->execute(array(':user_full_name' => $user_full_name, ':user_id' => $_SESSION['user_id']));
        $count =  $query->rowCount();
        if ($count == 1) {
            Session::set('user_full_name', $user_full_name);
            $_SESSION["feedback_positive"][] = FEEDBACK_FULLNAME_CHANGE_SUCCESSFUL;
            return true;
        } else {
            $_SESSION["feedback_negative"][] = FEEDBACK_UNKNOWN_ERROR;
            return false;
    ...
    public function changeLocation()
    {
        ...
        if (!isset($_POST['complexSelect']) OR empty($_POST['complexSelect'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_LOCATION_FIELD_EMPTY;
            return false;
        }

        // new location same as old one ?
        if ($_POST['complexSelect'] == $_SESSION["user_location"]) {
            $_SESSION["feedback_negative"][] = FEEDBACK_LOCATION_SAME_AS_OLD_ONE;
            return false;
        }

        // location cannot be empty and must be azAZ and 2-64 characters
        if (!preg_match("/^(?=.{2,64}$)[a-zA-Z ][a-zA-Z ]*(?: [a-zA-Z ]+)*$/", $_POST['complexSelect'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_LOCATION_DOES_NOT_FIT_PATTERN;
            return false;
        }

        // clean the input
        $user_location = substr(strip_tags($_POST['complexSelect']), 0, 64);

        $query = $this->db->prepare("UPDATE users SET user_location = :user_location WHERE user_id = :user_id");
        $query->execute(array(':user_location' => $user_location, ':user_id' => $_SESSION['user_id']));
        $count =  $query->rowCount();
        if ($count == 1) {
            Session::set('user_location', $user_location);
            $_SESSION["feedback_positive"][] = FEEDBACK_LOCATION_CHANGE_SUCCESSFUL;
            return true;
        } else {
            $_SESSION["feedback_negative"][] = FEEDBACK_UNKNOWN_ERROR;
            return false;
    ...
    public function registerNewUser()
    {
        ...
        if (empty($_POST['user_name'])) {
            ...
            $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_DOES_NOT_FIT_PATTERN;
		}elseif (empty($_POST['complexSelect'])) {
            $_SESSION["feedback_negative"][] = COMPLEX_FIELD_EMPTY; 
		}elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['complexSelect'])) {
            $_SESSION["feedback_negative"][] = COMPLEX_DOES_NOT_FIT_PATTERN;
        }elseif (empty($_POST['shift'])) {
            $_SESSION["feedback_negative"][] = COMPLEX_FIELD_EMPTY; 
		}elseif (!preg_match('/^[1-3\d]{1,2}$/i', $_POST['shift'])) {
            $_SESSION["feedback_negative"][] = COMPLEX_DOES_NOT_FIT_PATTERN;
        } 
		elseif (empty($_POST['user_email'])) {
			...
			$user_location = strip_tags($_POST['complexSelect']);
			$user_shift = strip_tags($_POST['shift']);
            ...
            $sql = "INSERT INTO users (user_name, user_full_name, user_active, user_password_hash, user_email, user_creation_timestamp, user_activation_hash, user_provider_type,  user_location, user_shift)
                    VALUES (:user_name, :user_full_name, :user_active, :user_password_hash, :user_email, :user_creation_timestamp, :user_activation_hash, :user_provider_type, :user_location, :user_shift)";
     ...
     */
    public function changeAccountType($id)
    {
		...
		elseif (isset($_POST["user_account_upgrade_admin"]) AND !empty($_POST["user_account_upgrade_admin"])) {
			   // upgrade account type
            $query = $this->db->prepare("UPDATE users SET user_account_type = 3 WHERE user_id = :user_id");
            $query->execute(array(':user_id' => $id));
