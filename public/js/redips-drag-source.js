...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		createObject,				//method to create an object
	...
	handlerOnMouseDown = function (e) {
		...
		cloneClass = obj.className.indexOf('CL') > -1 ? true : false;
	...
	rowClone = function (el, row_mode) {
		...
		// 2) rowClone call in onmousemove will clone current row (el.nodeName === 'TR')
			...
			if (cloneClass && cloned) {
				...
				div.className = normalize(div.className.replace('CL', ''));
			}
			// clone whole table
			tableMini = el.cloneNode(true);
			// return "clone" to the source element
			if (cloneClass && cloned) {
				div.className = div.className + ' CL';
			}
	...
	handlerOnMouseUp = function (e) {
		...
		// if DIV element was clicked and left button was released, but element is placed inside unmovable table cell
			...
			// remove object if destination cell has "trash" in class name
				...
				//ADDED a custom variable
				var trlName = document.getElementById(obj.id + 'nm').innerHTML;
	 ...
	 * @memberOf REDIPS.drag#
	 */
	elementDeleted = function (trlName, obj) {
		...
		REDIPS.drag.event.deleted(cloned, trlName, obj);
	};
				...
				if (cell_current.className.indexOf(REDIPS.drag.trash.className) === -1) {
					...
					// DIV objects marked with "only" can't be placed to other cells (if property "other" is "deny")
						...
						if ((mark_found === true && REDIPS.drag.mark.action === 'deny') || (mark_found === false && REDIPS.drag.mark.action === 'allow')) {
							...
							if (cell_current.className.indexOf(mark.exception[obj.id.substring(0, 3)]) === -1) {
								previous();
								break;
							}
						}
	...
	cloneObject = function (div, drag) {
		...
		cname = cname.replace('CL', '');
		cname = cname.replace(/climit(\d)_(\d+)/, '');
		var  touppercase = document.getElementById('CLtxt').value,
			 cloneText = touppercase.toUpperCase(),
			 cloneTextField = document.getElementById('CLtxt');
		
		
		if(/DT[0-9]/.test(cloneText.substring(0, 3)) || /SH[0-9]/.test(cloneText.substring(0, 3)) || /SP[0-9]/.test(cloneText.substring(0, 3)) || /NS[0-9]/.test(cloneText.substring(0, 3))) 
		{
			divCloned.className = normalize(cloneText.substring(0, 2) + ' ' + cname);
			divCloned.id = cloneText;
			divCloned.querySelector("#CLtxt").id = normalize('clonedtxt');
			divCloned.querySelector("h5").id = normalize(cloneText + 'nm');
			divCloned.querySelector("h5").innerHTML = normalize(cloneText);
			divCloned.querySelector("select").id = normalize(cloneText + 'op');
			divCloned.querySelector("div").id = normalize(cloneText + 'nt');
			divCloned.querySelector("#CLp").id = normalize(cloneText + 'p');
			divCloned.querySelector("#CLno").id = normalize(cloneText + 'no');
			divCloned.querySelector("#CLyd").id = normalize(cloneText + 'yd');
			divCloned.querySelector("#CLrd").id = normalize(cloneText + 'rd');
			for (var r = 0; r < 13; r++) {
				function pad(n) {
							return (n < 10) ? ("0" + n) : n;
				}
			divCloned.querySelector("#CLnoop" + pad(r)).id = normalize(cloneText + 'noop' + pad(r));
			divCloned.querySelector("#CLlb" + pad(r)).id = normalize(cloneText + 'lb' + pad(r));
			document.getElementById(cloneText + 'lb' + pad(r)).setAttribute('for', cloneText + 'noop' + pad(r));
			}

		}
		else if(cloneText.substring(1, 2) == '4') 
		{
			divCloned.className = normalize(cloneText.substring(0, 3) + ' ' + cname);
			divCloned.id = cloneText;
			divCloned.querySelector("#CLtxt").id = normalize('clonedtxt');
			divCloned.querySelector("h5").id = normalize(cloneText + 'nm');
			divCloned.querySelector("h5").innerHTML = normalize(cloneText);
		...
		else 
		{
			...
			divCloned.className = normalize('CO ' + cname);
			// if counter is undefined, set 0
				if (clonedId[div.id] === undefined) {
					clonedId[div.id] = 0;
				}
			//set text id to clonedtxt
			divCloned.querySelector("#CLtxt").id = normalize('clonedtxt');
			...
			divCloned.id =  ('CO' + clonedId[div.id]).toUpperCase() + Math.floor((Math.random() * 12430) + 2);
			
			if (cloneText == "" || cloneText == null) {
				cloneText = 'Outside Trailer ' + divCloned.id;
			}
			// increment clonedId for cloned element
			clonedId[div.id] += 1;
			divCloned.querySelector("h5").id = normalize(divCloned.id + 'nm');
			divCloned.querySelector("h5").innerHTML = normalize(cloneText);
			divCloned.querySelector("select").id = normalize(divCloned.id + 'op');
			divCloned.querySelector("div").id = normalize(divCloned.id + 'nt');
			divCloned.querySelector("#CLp").id = normalize(divCloned.id + 'p');
			divCloned.querySelector("#CLno").id = normalize(divCloned.id + 'no');
			divCloned.querySelector("#CLyd").id = normalize(divCloned.id + 'yd');
			divCloned.querySelector("#CLrd").id = normalize(divCloned.id + 'rd');
			for (var r = 0; r < 13; r++) {
				function pad(n) {
							return (n < 10) ? ("0" + n) : n;
				}
			divCloned.querySelector("#CLnoop" + pad(r)).id = normalize(divCloned.id + 'noop' + pad(r));
			divCloned.querySelector("#CLlb" + pad(r)).id = normalize(divCloned.id + 'lb' + pad(r));
			document.getElementById(divCloned.id + 'lb' + pad(r)).setAttribute('for', divCloned.id + 'noop' + pad(r));
			}
		}
		//clear textfield for the clone
	 ...
	 * @name REDIPS.drag#cloneObject
	 */
	createObject = function (div, objId) {
		var divCloned = div.cloneNode(true),	// cloned DIV element
			cname = divCloned.className,		// set class names of cloned DIV element
			offset,								// offset of the original object
			offsetDragged;						// offset of the new object (cloned)
		...
		cname = cname.replace('CL', '');
		cname = cname.replace(/climit(\d)_(\d+)/, '');
		var cloneText = objId.toUpperCase(),
			 cloneTextField = document.getElementById('CLtxt');
	...
	saveContent = function (tbl, type) {
		var query = '',						// define query parameter
			...
			id, r, c, d, tblNm = tbl,					// variables used in for loops
					...
					if (tbl_cell.childNodes.length > 0) {
						...
						for (d = 0; d < tbl_cell.childNodes.length; d++) {
							...
							if (cn.nodeName === 'DIV' && cn.className.indexOf('drag') > -1) { // and yes, it should be uppercase
								...
								query += pname + '[]=' + cn.id + '_' + r + '_' + c + '_' + tblNm + '&';
								// push values for DIV element as Array to the Array
								JSONobj.push([cn.id, r, c, tblNm]);
							}
						}
					}
				}
